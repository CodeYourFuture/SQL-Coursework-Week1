...
```sql
select t.date, t.supplier_id, t.description, t.amount from spends t  inner join expense_areas ea on  (ea.id = t.expense_area_id) where ea.expense_area = 'Better Hospital Food';

```
### 7. Show the date, supplier name, description and amount for transactions with the expense area of 'Better Hospital Food'. You will need to INNER JOIN another table to be able to do this.
```sql
 select t.date, s.supplier, t.description, t.amount from spends t inner join suppliers s on (s.id = t.supplier_id) inner join expense_areas ea on (t.expense_area_id = ea.id) where ea.expense_area = 'Better Hospital Food';
```
...
```sql
select date,sum(amount) as total_spent from spends group by date;
```
