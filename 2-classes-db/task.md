...
```sql
select date, transaction_no, description, supplier_inv_no, description, amount from spends where description like'%fee%';
```
...
-- another method select date, transaction_no, supplier_inv_no, description, amount from spends where lower(description) like lower('%fee%');
```
...
```sql
SELECT date as Month, sum(amount) as MonthlySpend
FROM spends
GROUP BY date;
```
