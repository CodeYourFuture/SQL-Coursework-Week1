...
from spends
where description like 'fee%' or description like '%fee' or description like '%fee%';
...
select date, transaction_no,supplier_inv_no, description, amount
from spends
where description ilike 'Fee%' or description ilike '%Fee' or description ilike '%Fee%';
...
```sql
select date as month, sum(amount)
