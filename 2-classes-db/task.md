...
SELECT expense_areas.id, spends.date, spends.supplier_id, spends.amount
FROM expense_areas
INNER JOIN spends ON expense_areas.id = spends.expense_area_id
...
--ERROR:  null value in column "amount" violates not-null constraint. The amount value is missing which means it will be null and it will throw this error.
```
