...
```sql
SELECT date, transation_no, supplier_inv_no, description, amount
FROM spends
WHERE description LIKE '%Fee%';
...
```sql
SELECT spends.date, spends.supplier_id, spends.description, spends.amount, expense_areas.expense_area
FROM spends
...
```sql
INSERT INTO spends (date, description, transaction_no, supplier_inv_no, supplier_id, expense_type_id, expense_area_id) VALUES ('2021-04-01', 'Computer Hardware Dell', 38104091, '3780119655', 16, 7, 18);
-- Note: this comment throws ERROR:  null value in column "amount" violates not-null constraint. Because the spends table does not accept amount value as null. Since the insert values do not contain amount value, sql assign amount cell as null. That throws the error written above. Adding amount in the first column parenthesis and then adding amount value in VALUES is the solution.
```
