...
```sql
SELECT date, supplier_id, description, amount FROM spends INNER JOIN expense_areas ON expense_areas.expense_area LIKE 'Better Hospital Food';
...
```sql
SELECT date, amount FROM spends GROUP BY spends.id, spends.date;
