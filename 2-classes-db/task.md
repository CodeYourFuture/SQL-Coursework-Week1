...
```sql
SELECT date, transaction_no, supplier_inv_no, description 
...
```sql
SELECT date, transaction_no,supplier_inv_no, description 
FROM spends WHERE LOWER(description) LIKE '%fee%';
```
...
SELECT date, supplier_id, description, amount
    ...
    WHERE expense_areas.expense_area 
    LIKE '%Better Hospital Food%';
```
...
//Part complete, my "£" is a "U" for some reason and I'm not sure about the date formatting, the answers are hard to parse online. 
