...
```sql
    SELECT date, description, transaction_no, amount FROM spends WHERE amount BETWEEN 30000 AND 31000;
```
...
```sql
    SELECT  date, suppliers.id, description,amount
    FROM spends 
...
```sql
    SELECT date, suppliers.supplier, amount, description
    FROM spends
    INNER JOIN expense_areas ON spends.expense_area_id = expense_areas.id
    INNER JOIN suppliers ON suppliers.id = spends.supplier_id
    WHERE expense_area='Better Hospital Food';
...
```sql
    INSERT INTO spends (expense_type_id, expense_area_id, supplier_id, date, transaction_no, supplier_inv_no, description, amount) VALUES (7,18,16,'2021-04-01',38104091,'3780119655','Computer Hardware Dell',81473);
...
```sql
            Select to_char(date, 'Mon YYYY') AS Month, to_char(SUM(amount),'£99,999,990') AS Monthly_Spend
            FROM spends 
            GROUP BY date;
```
