...
```sql
elect date, transaction_no, supplier_inv_no, description, amount from spends where description ilike '%fee%';
...
from spends inner join expense_areas on spends.expense_area_id = expense_areas.id
...
```sql
select date from spends group by date;
```
