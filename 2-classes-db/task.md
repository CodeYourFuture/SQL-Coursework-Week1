...
```sql
 SELECT date, supplier_id, description, amount FROM spends INNER JOIN expense_areas ON spends.expense_area_id = expense_areas.id AND expense_areas.expense_area LIKE 'Better Hospital Food';
```
...
on spends.expense_area_id = expense_areas.id and expense_areas.expense_area like 'Better Hospital Food';
```
...
```sql
SELECT to_char(date, 'Month YYYY'), SUM(amount) FROM spends GROUP BY to_char(date, 'Month YYYY');
```
