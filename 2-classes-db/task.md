...
```sql
SELECT * FROM spends;select (date, transaction_no, description, amount) from spends where amount > 30000 and amount < 31000;
```
...
INNER JOIN expense_areas ON expense_areas.id = spends.expense_area_id where expense_area= 'Better Hospital Food';
```
