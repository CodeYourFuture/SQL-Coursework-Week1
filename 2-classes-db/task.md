...
```sql
SELECT date, supplier as "supplier name", supplier_inv_no, description, amount 
FROM spends 
INNER JOIN expense_areas 
ON expense_areas.id = spends.expense_area_id 
...
(7, 18, 16, '01-05-2021', 38104091, '3780119655', 'Computer Hardware Dell', 32000)
```
...
```sql
SELECT CONCAT(TRIM(TO_CHAR(date, 'Month')),' ',DATE_PART('year', date)) AS "Month", CONCAT('£ ', TO_CHAR(SUM(amount), 'fm999G999G999G999')) AS "Monthly Spend"  
FROM spends 
GROUP BY CONCAT(TRIM(TO_CHAR(date, 'Month')),' ',DATE_PART('year', date));
```
