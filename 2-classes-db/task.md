...
```sql
SELECT s.date, sp.supplier, s.description, s.amount
FROM spends s
INNER JOIN expense_areas ea ON s.expense_area_id = ea.id
INNER JOIN suppliers sp ON s.supplier_id = sp.id
WHERE ea.expense_area = 'Better Hospital Food';
...
### 8. We have just received a late invoice for April! Add a new row to the spends table:
    ...
    for £32,000.
```sql
INSERT INTO spends (date, transaction_no, supplier_inv_no, description, supplier_id, expense_type_id, expense_area_id, amount)
VALUES ('2021-04-01', 38104091, '3780119655', 'Computer Hardware Dell', 16, 7, 18, 32000);
